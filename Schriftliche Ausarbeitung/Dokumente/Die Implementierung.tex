...
Indiesem Fall ist auch die Zeiger-Liste leer und keiner der Beweise ist komplexer als zwei simp.
...
Um matching\_nodeId zu zeigen, habe ich für einen generischen Knoten den entsprechenden Zeiger gebaut und musste anschließend zeigen, dass sich dieser auch in der Zeiger-Liste befindet.
\dots
% So, oder ist das zu viel wie und zu wenig warum?

\begin{lstlisting}
theorem init_unionFind.linkList.matching_nodeId {nodeList : List node} (h_nonempty : nodeList.length ≠ 0) : ∀ x ∈ nodeList, ∃! y, y ∈ (init_unionFind.linkList nodeList h_nonempty) ∧ x.id = y.nodeId
\end{lstlisting}
