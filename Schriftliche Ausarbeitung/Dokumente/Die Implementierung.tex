...
Die Beweise für die \KruskalunionFind Bedingungen habe ich ausgelagert.
Da diese sehr groß sind und auch eine Menge Argumente übergeben bekommen erwähne ich hier lediglich die Namen.

% update_unionFind_matching_nodeId
Bei \KruskalupdateunionFindmatchingnodeId geht es darum, dass es fürjeden Knoten genau einen Zeiger gibt.
Sei also $z$ ein beliebiger Knoten.
Ich mache eine Fallunterscheidung, ob $z$ gleich $x, y$ oder irgend ein anderer Knoten ist.
Im Fall, dass $z = x$ (bzw. $y$), zeige ich, dass erstens der neue Zeiger passt und Teil der Liste ist und zweitens alle anderen insbesondere der andere neue Zeiger nicht passen.
Im Fall, dass $z$ weder $x$ noch $y$ ist, zeige ich, dass sowohl die beide neuen als auch die beiden alten Zeiger von $x$
...
Für alle Elemente der alten Liste weis ich bereits, dass es gilt und die neuen Zeiger zeigen auf $x$ oder $y$, was auch einzigartige Knoten sind, weshalb es auch für die neuen gilt.
...
\KruskalupdateunionFindmatchingrank

% update_unionFind_rankInvariant
\KruskalupdateunionFindrankInvariant

% update_unionFind_rankInvariant_of_rank_eq
\KruskalupdateunionFindrankInvariantofrankeq

% update_unionFind_h_uFLx'_in
\KruskalupdateunionFindhuFLxprimein
