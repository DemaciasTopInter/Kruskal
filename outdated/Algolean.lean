...
structure updated (nodeList : List node) : Type where
  mk ::
  uF : unionFind nodeList
  rankInvariant' : uF.rankInvariant'
...
inductive find_query (nodeList : List node) : Type → Type _ where
  ...
  | union (uF : unionFind nodeList) (x y : Nat) (h₁ : ∃ a ∈ uF.linkList, (fun a => a.nodeId = x ∧ a.ccId = x) a) (h₂ : ∃ a ∈ uF.linkList, (fun a => a.nodeId = y ∧ a.ccId = y) a) (h_rankInvariant : uF.rankInvariant') : find_query nodeList (updated nodeList)

inductive kruskal_query' (nodeList : List node) : Type → Type _ where
  | find (uF : unionFind nodeList) (id : Nat) (h : ∃ x ∈ nodeList, x.id = id) (h_rankInvariant : uF.rankInvariant') : kruskal_query' nodeList (found nodeList uF)
  | union (uF : unionFind nodeList) (x y : Nat) (h₁ : ∃ a ∈ uF.linkList, (fun a => a.nodeId = x ∧ a.ccId = x) a) (h₂ : ∃ a ∈ uF.linkList, (fun a => a.nodeId = y ∧ a.ccId = y) a) (h_rankInvariant : uF.rankInvariant') : kruskal_query' nodeList (updated nodeList)
...
def kruskal_model3 (nodeList : List node) : Model (kruskal_query' nodeList) (Nat × Nat) where
  evalQuery
    | .find (uF : unionFind nodeList) id h _ =>
      ...
      have h_ex := exist_unionFindLink_of_connected_component_of_unionFind_of_id uF id h
      ⟨cc, h_ex⟩
    | .union (uF : unionFind nodeList) x y h₁ h₂ h_rankInvariant => ⟨update_unionFind uF x y h₁ h₂, update_unionFind.rankInvariant' _ _ _ _ _ h_rankInvariant⟩
  cost
...
def reduction {nodeList : List node} : Reduction (kruskal_query' nodeList) (find_query nodeList) where
  reduce
...
theorem model_evalQuery_eq {nodeList : List node} : ∀ {l} (q : (kruskal_query' nodeList) l), (reduction.reduce q).eval (find_model nodeList) = (kruskal_model3 nodeList).evalQuery q := by
  ...
  | find uF id h h_rankInvariant =>
    have h' : (reduction.reduce (kruskal_query'.find uF id h h_rankInvariant)).eval (find_model nodeList) = (connected_component_of_unionFind_of_id_prog uF id h).eval (find_model nodeList) := by
      simp [reduction]
...
theorem connected_component_of_unionFind_of_id_prog_tim_log {nodeList : List node} (uF : unionFind nodeList) (id : Nat) (h : ∃ x ∈ nodeList, x.id = id) (h_rankInvariant : uF.rankInvariant') : (connected_component_of_unionFind_of_id_prog uF id h).time (find_model nodeList) ≤ (Nat.log 2 nodeList.length, 0) := by
...
def kruskal_helper_prog' (edgeList : List edge) (nodeList : List node) (uF : unionFind nodeList) (edgesSoFar : List edge) (h_matching_edge : matching_edge edgeList nodeList) (h_nodup : nodeList.Nodup) (h_rankInvariant : uF.rankInvariant') : Prog (kruskal_query' nodeList) (List edge) := do
  match edgeList with
  | [] => return edgesSoFar
  | e::es =>
    have h : ∃ x ∈ nodeList, x.id = e.node1 := by
      have h : (∃ x ∈ nodeList, e.node1 = x.id) → (∃ x ∈ nodeList, x.id = e.node1) := by simp [eq_comm]
      simp [h (h_matching_edge e List.mem_cons_self).left]
    let found_x : (found nodeList uF) ← kruskal_query'.find uF e.node1 h h_rankInvariant
    let x := found_x.id
    have h' : ∃ x ∈ nodeList, x.id = e.node2 := by
      have h' : (∃ x ∈ nodeList, e.node2 = x.id) → (∃ x ∈ nodeList, x.id = e.node2) := by simp [eq_comm]
      simp [h' (h_matching_edge e List.mem_cons_self).right]
    let found_y : (found nodeList uF) ← kruskal_query'.find uF e.node2 h' h_rankInvariant
    let y := found_y.id
    have h_matching_edge' : ∀ x ∈ es, (∃ y ∈ nodeList, x.node1 = y.id) ∧ ∃ z ∈ nodeList, x.node2 = z.id := by
      intro z h_in
      simp [h_matching_edge z (List.mem_cons_of_mem e h_in)]
    let updated_uF : (updated nodeList) ← kruskal_query'.union uF x y found_x.ex found_y.ex h_rankInvariant
    let updated_edgesSoFar := update_edgesSoFar edgesSoFar e x y
    return ← kruskal_helper_prog' es nodeList updated_uF.uF updated_edgesSoFar h_matching_edge' h_nodup updated_uF.rankInvariant'
...
def kruskal_prog' (edgeList : List edge) (nodeList : List node) (h_matching_edge : matching_edge edgeList nodeList) (h_nodup : nodeList.Nodup) : Prog (kruskal_query' nodeList) (List edge) := do
    ...
    have h_matching_edge : matching_edge edgeListSorted nodeList := by
      ...
      simp [matching_edge] at h_matching_edge
      exact h_matching_edge e h_e_in
    have h_uF_rankInvariant : uF.rankInvariant' := by
      exact init_unionFind.rankInvariant' _
    return ← kruskal_helper_prog' edgeListSorted nodeList uF [] h_matching_edge h_nodup h_uF_rankInvariant

theorem kruskal_helper_eval3 (edgeList : List edge) (nodeList : List node) (uF : unionFind nodeList) (edgesSoFar : List edge) (h_matching_edge : matching_edge edgeList nodeList) (h_nodup : nodeList.Nodup) (h_rankInvariant : uF.rankInvariant')
  : kruskal_helper edgeList nodeList uF edgesSoFar h_matching_edge h_nodup = (kruskal_helper_prog' edgeList nodeList uF edgesSoFar h_matching_edge h_nodup h_rankInvariant).eval (kruskal_model3 nodeList) := by
  induction edgeList generalizing uF edgesSoFar with
  | nil =>
    simp [kruskal_helper, kruskal_helper_prog']
  | cons e edgeList ih =>
    ...
    set updated_edgesSoFar := update_edgesSoFar edgesSoFar e (connected_component_of_unionFind_of_id uF e.node1 (kruskal_helper._proof_1 nodeList e edgeList h_matching_edge)) (connected_component_of_unionFind_of_id uF e.node2 (kruskal_helper._proof_2 nodeList e edgeList h_matching_edge))
    simp [matching_edge] at h_matching_edge ih
    have ih := ih updated_uF updated_edgesSoFar h_matching_edge.right (update_unionFind.rankInvariant' _ _ _ _ _ h_rankInvariant)
...
theorem kruskal_eval3 (edgeList : List edge) (nodeList : List node) (h_matching_edge : matching_edge edgeList nodeList) (h_nodup : nodeList.Nodup)
  : kruskal edgeList nodeList h_matching_edge h_nodup = (kruskal_prog' edgeList nodeList h_matching_edge h_nodup).eval (kruskal_model3 nodeList) := by
  simp [kruskal, kruskal_prog']
  exact kruskal_helper_eval3 _ _ _ _ _ _ _

theorem kruskal_helper_time3_independent_edgesSoFar (edgeList : List edge) (nodeList : List node) (uF : unionFind nodeList) (edgesSoFar : List edge) (edgesSoFar' : List edge) (h_matching_edge : matching_edge edgeList nodeList) (h_nodup : nodeList.Nodup) (h_rankInvariant : uF.rankInvariant')
  : (kruskal_helper_prog' edgeList nodeList uF edgesSoFar h_matching_edge h_nodup h_rankInvariant).time (kruskal_model3 nodeList) = (kruskal_helper_prog' edgeList nodeList uF edgesSoFar' h_matching_edge h_nodup h_rankInvariant).time (kruskal_model3 nodeList) := by
  induction edgeList generalizing uF edgesSoFar edgesSoFar' with
  | nil =>
    simp [kruskal_helper_prog']
  | cons e edgeList ih =>
    simp [kruskal_helper_prog', kruskal_model3]
    set updated_uF := update_unionFind uF (connected_component_of_unionFind_of_id uF e.node1 (kruskal_helper._proof_1 nodeList e edgeList h_matching_edge)) (connected_component_of_unionFind_of_id uF e.node2 (kruskal_helper._proof_2 nodeList e edgeList h_matching_edge)) (kruskal_helper._proof_4 nodeList uF e (kruskal_helper._proof_1 nodeList e edgeList h_matching_edge)) (kruskal_helper._proof_5 nodeList uF e (kruskal_helper._proof_2 nodeList e edgeList h_matching_edge)) with ← h_updated_uF
    set updated_edgesSoFar := update_edgesSoFar edgesSoFar e (connected_component_of_unionFind_of_id uF e.node1 (kruskal_helper._proof_1 nodeList e edgeList h_matching_edge)) (connected_component_of_unionFind_of_id uF e.node2 (kruskal_helper._proof_2 nodeList e edgeList h_matching_edge)) with ← h_updated_edgesSoFar
    set updated_edgesSoFar' := update_edgesSoFar edgesSoFar' e (connected_component_of_unionFind_of_id uF e.node1 (kruskal_helper._proof_1 nodeList e edgeList h_matching_edge)) (connected_component_of_unionFind_of_id uF e.node2 (kruskal_helper._proof_2 nodeList e edgeList h_matching_edge)) with ← h_updated_edgesSoFar'
    have ih := ih updated_uF updated_edgesSoFar updated_edgesSoFar'
...
theorem kruskal_helper_time3_right (edgeList : List edge) (nodeList : List node) (uF : unionFind nodeList) (edgesSoFar : List edge) (h_matching_edge : matching_edge edgeList nodeList) (h_nodup : nodeList.Nodup) (n : Nat) (h_lt : (kruskal_helper edgeList nodeList uF edgesSoFar h_matching_edge h_nodup).length < n) (h_rankInvariant : uF.rankInvariant')
  : ((kruskal_helper_prog' edgeList nodeList uF edgesSoFar h_matching_edge h_nodup h_rankInvariant).time (kruskal_model3 nodeList)).2 ≤ n := by
    induction edgeList generalizing uF edgesSoFar n with
    | nil =>
      simp [kruskal_helper_prog']
    | cons e edgeList ih =>
      simp [kruskal_helper_prog', kruskal_model3, ← add_assoc]
      set updated_uF := update_unionFind uF (connected_component_of_unionFind_of_id uF e.node1 (kruskal_helper._proof_1 nodeList e edgeList h_matching_edge)) (connected_component_of_unionFind_of_id uF e.node2 (kruskal_helper._proof_2 nodeList e edgeList h_matching_edge)) (kruskal_helper._proof_4 nodeList uF e (kruskal_helper._proof_1 nodeList e edgeList h_matching_edge)) (kruskal_helper._proof_5 nodeList uF e (kruskal_helper._proof_2 nodeList e edgeList h_matching_edge)) with ← h_updated_uF
      set updated_edgesSoFar := update_edgesSoFar edgesSoFar e (connected_component_of_unionFind_of_id uF e.node1 (kruskal_helper._proof_1 nodeList e edgeList h_matching_edge)) (connected_component_of_unionFind_of_id uF e.node2 (kruskal_helper._proof_2 nodeList e edgeList h_matching_edge)) with ← h_updated_edgesSoFar
      simp [matching_edge] at h_matching_edge ih
      split_ifs with h_con
      · simp
        simp [kruskal_helper, h_updated_uF, h_updated_edgesSoFar] at h_lt
        have ih := ih updated_uF updated_edgesSoFar h_matching_edge.right n h_lt (update_unionFind.rankInvariant' _ _ _ _ _ h_rankInvariant)
        simp [kruskal_model3] at ih
        apply le_of_eq_of_le ?_ ih
        simp [h_updated_uF]
      · simp
        ...
        simp
        simp [← List.length_append, ← kruskal_helper.edgesSoFar_append, h_n'] at h_lt
        have ih := ih updated_uF edgesSoFar h_matching_edge.right n' h_lt (update_unionFind.rankInvariant' _ _ _ _ _ h_rankInvariant)
        simp [kruskal_model3] at ih
        have h_eq := kruskal_helper_time3_independent_edgesSoFar edgeList nodeList updated_uF edgesSoFar updated_edgesSoFar h_matching_edge.right h_nodup (update_unionFind.rankInvariant' _ _ _ _ _ h_rankInvariant)
        simp [kruskal_model3, ← h_updated_uF, ← h_updated_edgesSoFar] at h_eq
        rw [← h_eq]
        exact ih

theorem kruskal_helper_time3 (edgeList : List edge) (nodeList : List node) (uF : unionFind nodeList) (edgesSoFar : List edge) (h_matching_edge : matching_edge edgeList nodeList) (h_nodup : nodeList.Nodup) (h_nonempty : nodeList ≠ []) (h_length_lt : (kruskal_helper edgeList nodeList uF edgesSoFar h_matching_edge h_nodup).length < (nodeList.max h_nonempty).id.succ) (h_rankInvariant : uF.rankInvariant')
  : (kruskal_helper_prog' edgeList nodeList uF edgesSoFar h_matching_edge h_nodup h_rankInvariant).time (kruskal_model3 nodeList) ≤ (2 * edgeList.length * (Nat.log 2 nodeList.length), (nodeList.max h_nonempty).id.succ) := by
  set n := (nodeList.max h_nonempty).id.succ with ← h_n
  constructor
  · simp
    induction edgeList generalizing uF edgesSoFar with
    | nil =>
      simp [kruskal_helper_prog']
    | cons e edgeList ih =>
      simp [kruskal_helper_prog', kruskal_model3, ← add_assoc, mul_add]
      simp [add_comm, add_assoc]
      set updated_uF := update_unionFind uF (connected_component_of_unionFind_of_id uF e.node1 (kruskal_helper._proof_1 nodeList e edgeList h_matching_edge)) (connected_component_of_unionFind_of_id uF e.node2 (kruskal_helper._proof_2 nodeList e edgeList h_matching_edge)) (kruskal_helper._proof_4 nodeList uF e (kruskal_helper._proof_1 nodeList e edgeList h_matching_edge)) (kruskal_helper._proof_5 nodeList uF e (kruskal_helper._proof_2 nodeList e edgeList h_matching_edge))
      set updated_edgesSoFar := update_edgesSoFar edgesSoFar e (connected_component_of_unionFind_of_id uF e.node1 (kruskal_helper._proof_1 nodeList e edgeList h_matching_edge)) (connected_component_of_unionFind_of_id uF e.node2 (kruskal_helper._proof_2 nodeList e edgeList h_matching_edge))
      simp [matching_edge] at h_matching_edge ih
      have h_length_lt' : (kruskal_helper edgeList nodeList updated_uF updated_edgesSoFar h_matching_edge.right h_nodup).length < n := by
        simp [kruskal_helper] at h_length_lt
        grind
      have ih := ih updated_uF updated_edgesSoFar h_matching_edge.right (update_unionFind.rankInvariant' _ _ _ _ _ h_rankInvariant) h_length_lt'
...
theorem kruskal_time3 (edgeList : List edge) (nodeList : List node) (h_matching_edge : matching_edge edgeList nodeList) (h_nonempty : nodeList ≠ []) (h_nonempty' : edgeList ≠ []) (h_nodeList : nodeList = nodeList_of_edgeList edgeList) (h_nodup' : edgeList.Nodup) (h_nodup_con : ∀ e1 ∈ edgeList, ∀ e2 ∈ edgeList, e1.node1 = e2.node1 ∧ e1.node2 = e2.node2 ∨ e1.node1 = e2.node2 ∧ e1.node2 = e2.node1 → e1 = e2)
  : (kruskal_prog' edgeList nodeList h_matching_edge (by rw[h_nodeList]; exact nodeList_of_edgeList_nodup edgeList)).time (kruskal_model3 nodeList) ≤ (2 * edgeList.length * (Nat.log 2 nodeList.length), (nodeList.max h_nonempty).id.succ) := by
  have h_nodup : nodeList.Nodup := by
    rw[h_nodeList]
    exact nodeList_of_edgeList_nodup edgeList
  simp [kruskal_prog']
  ...
  have h_rankInvariant : uF.rankInvariant' := init_unionFind.rankInvariant' _
